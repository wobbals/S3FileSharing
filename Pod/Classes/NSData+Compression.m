...
//  Modifications to remove the OpenSSL dependency by Charley Robinson
//
...
+ (NSData *) dataWithBase32String:(NSString *)encoded
{
    ...
    // avoids using characterAtIndex.
    const char *chars = [encoded cStringUsingEncoding:NSASCIIStringEncoding];
    NSUInteger charsLen =
    [encoded lengthOfBytesUsingEncoding:NSASCIIStringEncoding];
    
    // Note that the code below could detect non canonical Base32 length within
    // the loop. However canonical Base32 length can be tested before entering
    // the loop.
...
{
    NSData* md5 = [self md5Digest];
    const uint8_t* md5Buffer = md5.bytes;
    
    NSMutableString *output =
    [NSMutableString stringWithCapacity:md5.length * 2];
...
- (NSData*) sha1Digest
{
    unsigned char md5Buffer[CC_SHA1_DIGEST_LENGTH];
    CC_SHA1(self.bytes, (unsigned int) self.length, md5Buffer);
    return [NSData dataWithBytes:md5Buffer length:CC_SHA1_DIGEST_LENGTH];
}

- (NSString*) sha1DigestString
{
    NSData* md5 = [self sha1Digest];
    const uint8_t* md5Buffer = md5.bytes;
